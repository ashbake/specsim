...
######################
    ...
    def observe(self):
           ...
           #  calc noise
            ...
            s_frame = self.obs_s_frame
            s_frame[np.where(np.isnan(s_frame))] = np.inf
            s_frame[np.where(s_frame==0)] = np.inf
            self.obs_s_frame = s_frame

            self.nframe = (self.obs_noise_frame / self.obs_s_frame)**2 * self.SN**2
            self.expt = self.nframe * self.exp_frame
            print(self.nframe)
            print(np.max(self.nframe))
            print(np.mean(self.nframe))
