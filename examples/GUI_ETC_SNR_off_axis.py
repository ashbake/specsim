...
#
        ...
        self.textEdit_4_exp_frame = QPlainTextEdit(self.window)
        self.textEdit_4_exp_frame.setPlaceholderText("s")
        self.textEdit_4_exp_frame.move(740,50)
        self.textEdit_4_exp_frame.resize(50,20)
#
        self.label_4_exp_frame = QLabel(self.window)
        self.label_4_exp_frame.setText('Frame exp time:')
        self.label_4_exp_frame.move(610,50)
        self.label_4_exp_frame.resize(140,20)

        self.textEdit_4_SNR = QPlainTextEdit(self.window)
        self.textEdit_4_SNR.setPlaceholderText("")
        self.textEdit_4_SNR.move(740,75)
        self.textEdit_4_SNR.resize(50,20)
#
        self.label_4_SNR = QLabel(self.window)
        self.label_4_SNR.setText('Target SNR:')
        self.label_4_SNR.move(610,75)
        self.label_4_SNR.resize(140,20)
    ...
    def parameter4(self):
        self.exp_frame = float(self.textEdit_4_exp_frame.toPlainText())
        self.SN = float(self.textEdit_4_SNR.toPlainText())
...
######################
    ...
    def observe_D(self):
           ...
           #  calc noise
            ...
            p_frame = self.obs_p_frame
            p_frame[np.where(np.isnan(p_frame))] = np.inf
            p_frame[np.where(p_frame==0)] = np.inf
            self.obs_p_frame = p_frame

            self.nframe = (self.obs_noise_frame / self.obs_p_frame)**2 * self.SN**2
            self.expt = self.nframe * self.exp_frame

            print(self.obs_noise_frame)
            print(np.max(self.obs_noise_frame))
            print(np.mean(self.obs_noise_frame))
            print(len(self.obs_noise_frame))
            print(self.obs_p_frame)
            print(np.max(self.obs_p_frame))
            print(np.min(self.obs_p_frame))
            print(np.mean(self.obs_p_frame))
            print(len(self.obs_p_frame))
