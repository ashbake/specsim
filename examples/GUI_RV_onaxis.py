...
######################
    ...
    def observe(self):
        ...
        if self.pl_on == 1: # 3 port lantern hack
            noise_frame_yJ  = np.sqrt(3) * np.array(self.sum_total_noise(self.obs_s_frame/3,self.exp_frame, self.nsamp,self.obs_inst_bg_ph, self.obs_sky_bg_ph, self.dark,self.read,self.pix_vert))
    ...
    def get_order_bounds(self):
        ...
        order_cen_lam	= self.stel_v[order_peaks[0]]
        blaze_angle	  =  76
        order_indices	=[]
    ...
    def make_telluric_mask(self):
        ...
        telluric_spec_lores = self.degrade_spec(self.stel_v, telluric_spec, self.inst_res)
        # resample onto v array
        filt_interp	 = interpolate.interp1d(self.stel_v, telluric_spec_lores, bounds_error=False,fill_value=0)
    ...
    def get_rv_content(self):
        """
        """
        flux_interp = interpolate.InterpolatedUnivariateSpline(self.obs_v,self.obs_s, k=1)
