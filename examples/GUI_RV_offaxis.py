...
class obs_snr_off():
    def __init__(self):
        ...
        self.textEdit_1_teff_p = QPlainTextEdit(self.window)
        self.textEdit_1_teff_p.setPlaceholderText("Kelvin")
        self.textEdit_1_teff_p.move(140,175)
        self.textEdit_1_teff_p.resize(50,20)

        self.label_1_teff_p = QLabel(self.window)
        self.label_1_teff_p.setText('Planet Temperature:')
        self.label_1_teff_p.move(10,175)
        self.label_1_teff_p.resize(140,20)


        self.textEdit_1_mag_p = QPlainTextEdit(self.window)
        self.textEdit_1_mag_p.setPlaceholderText("Vega")
        self.textEdit_1_mag_p.move(140,200)
        self.textEdit_1_mag_p.resize(50,20)

        self.label_1_mag_p = QLabel(self.window)
        self.label_1_mag_p.setText('Magnitude of Planet:')
        self.label_1_mag_p.move(10,200)
        self.label_1_mag_p.resize(140,20)

        self.textEdit_1_family_p = QPlainTextEdit(self.window)
        self.textEdit_1_family_p.setPlaceholderText("filtername")
        self.textEdit_1_family_p.move(140,225)
        self.textEdit_1_family_p.resize(50,20)

        self.label_1_family_p = QLabel(self.window)
        self.label_1_family_p.setText('Filter family:')
        self.label_1_family_p.move(10,225)
        self.label_1_family_p.resize(140,20)

        self.textEdit_1_band_p = QPlainTextEdit(self.window)
        self.textEdit_1_band_p.setPlaceholderText("filterband")
        self.textEdit_1_band_p.move(140,250)
        self.textEdit_1_band_p.resize(50,20)

        self.label_1_band_p = QLabel(self.window)
        self.label_1_band_p.setText('Filter band:')
        self.label_1_band_p.move(10,250)
        self.label_1_band_p.resize(140,20)

        self.textEdit_1_vsini_p = QPlainTextEdit(self.window)
        self.textEdit_1_vsini_p.setPlaceholderText("km/s")
        self.textEdit_1_vsini_p.move(140,275)
        self.textEdit_1_vsini_p.resize(50,20)

        self.label_1_vsini_p = QLabel(self.window)
        self.label_1_vsini_p.setText('Vsini of Planet:')
        self.label_1_vsini_p.move(10,275)
        self.label_1_vsini_p.resize(140,20)

        self.textEdit_1_rv_p = QPlainTextEdit(self.window)
        self.textEdit_1_rv_p.setPlaceholderText("km/s")
        self.textEdit_1_rv_p.move(140,300)
        self.textEdit_1_rv_p.resize(50,20)

        self.label_1_rv_p = QLabel(self.window)
        self.label_1_rv_p.setText('RV of Planet:')
        self.label_1_rv_p.move(10,300)
        self.label_1_rv_p.resize(140,20)

        self.textEdit_1_ang_sep = QPlainTextEdit(self.window)
        self.textEdit_1_ang_sep.setPlaceholderText("mas")
        self.textEdit_1_ang_sep.move(140,325)
        self.textEdit_1_ang_sep.resize(50,20)

        self.label_1_ang_sep = QLabel(self.window)
        self.label_1_ang_sep.setText('Angular Seperation:')
        self.label_1_ang_sep.move(10,325)
        self.label_1_ang_sep.resize(140,20)

        #############

        self.textEdit_1_minwv = QPlainTextEdit(self.window)
        self.textEdit_1_minwv.setPlaceholderText("nm")
        self.textEdit_1_minwv.move(140,350)
        self.textEdit_1_minwv.resize(50,20)

        self.label_1_minwv = QLabel(self.window)
        self.label_1_minwv.setText('Wavelength, from:')
        self.label_1_minwv.move(10,350)
...
#
        ...
        self.button_1.clicked.connect(self.planet)
        ...
        self.button_2.clicked.connect(self.get_speckle_noise)
        ...
        self.button_4.clicked.connect(self.sum_total_noise_D)
        self.button_4.clicked.connect(self.observe_D)
        self.button_4.clicked.connect(self.get_order_bounds)
        self.button_4.clicked.connect(self.make_telluric_mask)
        self.button_4.clicked.connect(self.get_rv_content)
        self.button_4.clicked.connect(self.get_rv_precision)
    ...
    def parameter1(self):
        ...
        self.teff_p = float(self.textEdit_1_teff_p.toPlainText())
        self.mag_p = float(self.textEdit_1_mag_p.toPlainText())
        self.vsini_p = float(self.textEdit_1_vsini_p.toPlainText())
        self.rv_p = float(self.textEdit_1_rv_p.toPlainText())
        self.band_p = self.textEdit_1_band_p.toPlainText()        
        self.family_p = self.textEdit_1_family_p.toPlainText()
        self.ang_sep = float(self.textEdit_1_ang_sep.toPlainText())
    ...
    def parameter2(self):
        ...
        if self.instrument =='modhis':
            ...
            self.nsamp = 16.0
            self.p_law_dh =-2.0
            self.telescope_seeing=0.6
            self.nactuators =58.0
            self.fiber_contrast_gain =10.0
            ...
            self.p_law_dh =-2.0
            self.telescope_seeing=0.6
            self.nactuators =30.0
            self.fiber_contrast_gain =3.0
    ...
    def load_spec_model(self,teff_input,wav_start=750,wav_end=780):
        if type(teff_input)==bool:
            return teff_input=='1'
        else:
            if teff_input>2300.0:
                teff = str(int(teff_input)).zfill(5)
                print(teff)
    ...
    def scale_stellar(self,mag,teff):
        """
        ...
        """
        stelv,stels       =  self.load_spec_model(teff_input=teff,wav_start=np.min(self.filt_xraw), wav_end=np.max(self.filt_xraw)) #phot/m2/s/nm
    ...
    def planet(self):
        ...
        #
        self.plan_vraw,self.plan_sraw = self.load_spec_model(teff_input=self.teff_p,wav_start=self.inst_l0,wav_end=self.inst_l1)
        self.plan_v   = self.x
        tck_stel    = interpolate.splrep(self.plan_vraw,self.plan_sraw, k=2, s=0)
        self.plan_s   = interpolate.splev(self.x,tck_stel,der=0,ext=1)

        # apply scaling factor to match filter zeropoint
        self.plan_factor_0   = self.scale_stellar(teff=self.teff_p,mag=self.mag_p) 
        self.plan_s   *= self.plan_factor_0
        self.plan_units = 'photons/s/m2/nm' # stellar spec is in photons/s/m2/nm
        ...
        if self.vsini_p > 0:
            ...
            vsini_kernel,_ = self._lsf_rotate(dvel_mean,self.vsini_p,epsilon=0.6)
            flux_vsini     = convolve(self.plan_s,vsini_kernel,normalize_kernel=True)  # photons / second / Ang
            self.plan_s      = flux_vsini
        if self.rv_p > 0:
            dvelocity =   (self.plan_v / 300000) * u.nm * consts.c / (self.plan_v * u.nm )
            rv_shift_resel = np.mean(self.rv_p * u.km / u.s / dvelocity) * 1000*u.m/u.km
            spec_shifted = shift(self.plan_s,rv_shift_resel.value)
            self.plan_s      = spec_shifted
        
        print('factor 0:',self.plan_factor_0)
        print('obj photon:',self.plan_s)
        print('obj wvs:',self.plan_v)
    ...
    def stellar(self):
        ...
        # Part 1: load raw spectrum
        #
        self.stel_vraw,self.stel_sraw = self.load_spec_model(teff_input=self.teff,wav_start=self.inst_l0,wav_end=self.inst_l1)
        self.stel_v   = self.x
        ...
        # apply scaling factor to match filter zeropoint
        self.stel_factor_0   = self.scale_stellar(teff=self.teff,mag=self.mag) 
    ...
    def get_band_mag(self,band_ao):
        ...
        if band_ao ==False:
            ...
            if (np.min(x) < self.inst_l0) or (np.max(x) > self.inst_l1):
                vraw,sraw = self.load_spec_model(teff_input=self.teff,wav_start=np.min(x),wav_end=np.max(x)) #phot/m2/s/nm
...
######################
    ...
    def get_speckle_noise(self):
        ...
        separations = (self.ang_sep * u.mas).to(u.arcsec)
        ao_mag=self.ao_ho_wfe_mag
        wvs=self.inst_xtransmit/1000 * u.um
        ...
        r0 = 0.55e-6/((self.telescope_seeing*u.arcsec).to(u.arcsecond).value/206265) * u.m #Easiest to ditch the seeing unit here. 
        ...
        w_halo = self.tel_diam * u.m / r0_sc

        for i,sep in enumerate(separations):
            ang_sep_resel_in = sep/206265/u.arcsecond*self.tel_diam * u.m /wvs.to(u.m) #Convert separtiona from arcsec to units of lam/D
        ...
        self.coron_contrast= contrast

        print(self.coron_contrast)
        print(np.max(self.coron_contrast))
        print(np.mean(self.coron_contrast))

                
    def sum_total_noise_D(self,flux,star_flux,texp, nsamp, inst_bg, sky_bg,darknoise,readnoise,npix,contrast,noisecap=None):
        """
        noise in 1 exposure
        ...
        noise = (flux + total_bg + npix * sig_read**2 + (darknoise * npix * texp)+speckle_noise + speckle_noise**2 )**(1/2)
    ...
    def observe_D(self):
            ...
            flux_per_sec_nm_p = self.plan_s  * self.tel_area * self.inst_ytransmit * np.abs(self.tel_s)
            ...
            self.flux_per_nm_p = flux_per_sec_nm_p * self.exp_frame
            s_ccd_lores_p = self.degrade_spec(self.plan_v, self.flux_per_nm_p, self.inst_res)

            # resample onto res element grid
            self.obs_v, self.obs_p_frame = self.resample(self.stel_v,s_ccd_lores_p,sig=self.inst_sig, dx=0, eta=1,mode='variable')
            self.obs_p_frame *= 0.925 # extraction fraction, reduce photons
            self.obs_p =  self.obs_p_frame * self.nframes
            ...
            instrument_contrast_interp= interpolate.interp1d(self.inst_xtransmit,self.coron_contrast)
            ...
            self.coron_inst_contr = instrument_contrast_interp(self.obs_v)[0]
           ...
           #  calc noise
            if self.pl_on == 1: # 3 port lantern hack
                noise_frame_yJ  = np.sqrt(3) * np.array(self.sum_total_noise_D(self.obs_p_frame/3,self.obs_s_frame/3,self.exp_frame, self.nsamp,self.obs_inst_bg_ph, self.obs_sky_bg_ph, self.dark,self.read,self.pix_vert,self.coron_inst_contr))
                noise_frame     = self.sum_total_noise_D(self.obs_p_frame,self.obs_s_frame,self.exp_frame, self.nsamp,self.obs_inst_bg_ph,self.obs_sky_bg_ph,self.dark,self.read,self.pix_vert,self.coron_inst_contr)
                yJ_sub          = np.where(self.obs_v < 1400)[0]
                noise_frame[yJ_sub] = noise_frame_yJ[yJ_sub] # fill in yj with sqrt(3) times noise in PL case
            else:
                noise_frame  = self.sum_total_noise_D(self.obs_s_frame,self.obs_p_frame,self.exp_frame,self.nsamp,self.obs_inst_bg_ph,self.obs_sky_bg_ph,self.dark,self.read,self.pix_vert,self.coron_inst_contr)
    ...
    def get_order_bounds(self):
        """
        given array, return max and mean of snr per order
        """
        #if self.rv_line_spacing == 'None':
        line_spacing= None ###hardcode
        #else :
        #    line_spacing=so.rv.line_spacing

        peak_spacing=2e4 ###hardcode
        height=0.055 ###hardcode
        order_peaks	  = signal.find_peaks(self.base_throughput,height=height,distance=peak_spacing,prominence=0.01)
        order_cen_lam	= self.plan_v[order_peaks[0]]
        blaze_angle	  =  76
        order_indices	=[]
        for i,lam_cen in enumerate(order_cen_lam):
            ...
            fsr  = lam_cen/m
            order_indices.append(np.where((self.obs_v > (lam_cen - 0.9*fsr/2)) & (self.obs_v  < (lam_cen+0.9*fsr/2)))[0])

        self.rv_order_cen_lam=order_cen_lam
        self.rv_order_indices=order_indices

    def make_telluric_mask(self):
        """
        """
        cutoff=0.01 ###hard
        velocity_cutoff=int(10) ###hard
        water_only=False ###hard
        telluric_spec = np.abs(self.tel_s/self.tel_rayleigh)**self.airmass
        #if water_only: telluric_spec = np.abs(self.tel_h2o)**self.airmass #h2o only
        telluric_spec[np.where(np.isnan(telluric_spec))] = 0
        telluric_spec_lores = self.degrade_spec(self.plan_v, telluric_spec, self.inst_res)
        # resample onto v array
        filt_interp	 = interpolate.interp1d(self.plan_v, telluric_spec_lores, bounds_error=False,fill_value=0)
        s_tel		 = filt_interp(self.obs_v)/np.max(filt_interp(self.obs_v))	# filter profile resampled to phoenix times phoenix flux density
        ...
        self.rv_telluric_mask=telluric_mask
        self.rv_s_tel=s_tel
    def get_rv_content(self):
        """
        """
        flux_interp = interpolate.InterpolatedUnivariateSpline(self.obs_v,self.obs_p, k=1)
        dflux = flux_interp.derivative()
        spec_deriv = dflux(self.obs_v)
        sigma_ord = np.abs(self.obs_noise) #np.abs(s) ** 0.5 # np.abs(n)
        sigma_ord[np.where(sigma_ord ==0)] = 1e10
        all_w = (self.obs_v ** 2.) * (spec_deriv ** 2.) / sigma_ord ** 2. # include read noise and dark here!!
        
        self.rv_all_w=all_w

    def get_rv_precision(self):
        SPEEDOFLIGHT = 2.998e8 # m/s
        all_w=self.rv_all_w
        order_cens=self.rv_order_cen_lam
        order_inds=self.rv_order_indices
        noise_floor=0.5 ###hard
        mask=self.rv_telluric_mask
        if np.any(mask==None):
        ...
        self.rv_dv_tot=dv_tot
        self.rv_dv_spec=dv_spec
        self.rv_dv_vals=dv_vals

        print(np.mean(self.rv_dv_tot))
        print(np.mean(self.rv_dv_spec))
        print(np.mean(self.rv_dv_vals))
