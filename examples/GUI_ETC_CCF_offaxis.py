...
#
        ...
        self.textEdit_4_goal_ccf = QPlainTextEdit(self.window)
        self.textEdit_4_goal_ccf.setPlaceholderText("")
        self.textEdit_4_goal_ccf.move(740,75)
        self.textEdit_4_goal_ccf.resize(50,20)
#
        self.label_4_goal_ccf = QLabel(self.window)
        self.label_4_goal_ccf.setText('Goal CCF:')
        self.label_4_goal_ccf.move(610,75)
        self.label_4_goal_ccf.resize(140,20)
    ...
    def parameter4(self):
        ...
        self.goal_ccf = float(self.textEdit_4_goal_ccf.toPlainText())
...
######################
    ...
    def observe_D(self):
            ...
            flux_per_sec_nm_p_nosky = self.plan_s  * self.tel_area * self.inst_ytransmit
            ...
            self.flux_per_nm_p_nosky = flux_per_sec_nm_p_nosky * self.exp_frame
            s_ccd_lores_p_nosky = self.degrade_spec(self.plan_v, self.flux_per_nm_p_nosky, self.inst_res)

            # resample onto res element grid
            self.obs_v, self.obs_p_nosky_frame = self.resample(self.stel_v,s_ccd_lores_p_nosky,sig=self.inst_sig, dx=0, eta=1,mode='variable')
            self.obs_p_nosky_frame *= 0.925 # extraction fraction, reduce photons
            self.obs_p_nosky =  self.obs_p_nosky_frame * self.nframes
###keep^
            ...
            speckle_noise = self.obs_s_frame * self.coron_inst_contr
            self.obs_speckle_noise = speckle_noise * np.sqrt(self.nframes)
            self.obs_thermal =(self.obs_inst_bg_ph + self.obs_sky_bg_ph) * self.exp_frame * self.nframes
           #  calc noise
            ...
            self.obs_noise = np.sqrt(self.nframes)*noise_frame #so.obs.noise_p

            ccf_cal = 0.01

            sky_trans = np.interp(self.obs_v,self.stel_v,self.tel_s)
            dwvs = np.abs(self.obs_s - np.roll(self.obs_s, 1))
            dwvs[0] = dwvs[1]
            ...
            obs_time = self.exp_time

            signal=self.obs_p
            model=self.obs_p_nosky
            sky_trans=sky_transmission_lsf
            systematics_residuals=ccf_cal
            kernel_size=501
            norm_cutoff=0.8
            signal = signal / obs_time
            model  = model / obs_time
            photon_noise = np.sqrt(self.obs_thermal+self.obs_p+self.obs_speckle_noise)
            ...
            min_exp_time = self.goal_ccf**2 / ((np.sum(signal_filt * model_filt/total_noise_flux))**2 / np.sum(model_filt * model_filt/total_noise_flux))
            self.obs_etc_ccf = min_exp_time
			#Calculate some normalization factor
			#Dimitri to explain this better. 

            #self.obs_snr = self.obs_s/self.obs_noise
            print((self.obs_etc_ccf))
            print(np.min(self.obs_etc_ccf))
            print(np.max(self.obs_etc_ccf))
            print(np.mean(self.obs_etc_ccf))
        ...
        #    print(np.max(self.obs_p))
        #    print(np.mean(self.obs_p))
        #    print(np.max(self.obs_noise))
        #    print(np.min(self.obs_noise))
